...
<%-- 
	...
	step : 증감 값(default 1)
	var : 인덱스 변수 or 향상된 for문에서 사용될 경우 conllection 객체에서 꺼낸 값 설정
	varStatus : 루프변수
		index - 현재 인덱스 값
		count - 루프 실행 횟수
		being, end - 루프의 being, end 값
 --%>
  <h3>일반 for 문형태</h3>
  ...
  %>
  	<c:forEach items="${rangers }" var="ranger" varStatus="loop">
  		${ranger } / ${loop.index } / ${loop.count } / ${loop.begin } / ${loop.end }<br> 뒤에 두갠 안되는거
  	</c:forEach>
  	...
  	var 속성을 적용한 경우 : var 속성으로 저장만 된다 (el을 통해 활용)<c:url value="/userRegist" var="url"/>
  	${url }
  	<br>
  	<a href="${url }">urlrlrl</a>
</body>
</html>